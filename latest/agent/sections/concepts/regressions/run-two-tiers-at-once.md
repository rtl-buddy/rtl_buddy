...
One regression per simulator in one checkout, concurrently, needs a per-run artefact namespace — otherwise both runs write the same `artefacts/<test>/` paths and the second dies on the [artefact-tree lock](https://rtl-buddy.github.io/rtl_buddy/v6/concepts/execution-context/#handle-an-artefact-lock). Give each run a `--run-tag`:
