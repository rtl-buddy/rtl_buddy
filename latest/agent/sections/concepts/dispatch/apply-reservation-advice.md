...
A suite that [splits verilation off](https://rtl-buddy.github.io/rtl_buddy/v6/concepts/dispatch/#split-verilation-from-the-c-build) gets a second row, `(verilate job)` with `phase: verilate`, read the same way; the `(build job)` row then describes the C++ build job alone.
