...
When the P&R run's PDK sets [`rcx-rules`](https://rtl-buddy.github.io/rtl_buddy/v6/concepts/pnr/#tune-the-process-dependent-steps), `rb pnr` writes an OpenRCX-extracted `<top>.routed.spef` beside the ODB, and a `netlist-source: pnr` power run reads it with `read_spef` after the routed SDC, in place of `estimate_parasitics -global_routing`. Without one — including the template's Nangate45 runs, which leave the key unset — the ODB handoff and the global-route estimate are used exactly as before.
