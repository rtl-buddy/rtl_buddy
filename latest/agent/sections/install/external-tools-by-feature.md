...
| `phys` | None | Reads the `phys-model.json` and `phys-manifest.json` that `rb synth` and `rb power` write; runs no external tool. See [Physical Metrics](https://rtl-buddy.github.io/rtl_buddy/v6/concepts/phys/). |
