...
Wire it into `rb synth` by setting `opts.frontend: "slang"` and `opts.plugin-path` under `cfg-synth-tools` (see [`SystemVerilog frontend`](https://rtl-buddy.github.io/rtl_buddy/v5/concepts/synthesis/#systemverilog-frontend) below). Skip this step entirely if your designs work with the default `frontend: "verilog"`.
