...
| `rb hub install-launchagent` | (macOS) Install a LaunchAgent so the hub auto-starts at login. See [Auto-start on macOS](https://rtl-buddy.github.io/rtl_buddy/v5/concepts/hub/#auto-start-on-macos-launchagent). |
...
| `rb hub send <verb> …` | One-shot peer that connects as `origin=cli` to drive the running hub from scripts. See [Driving the hub from the CLI](https://rtl-buddy.github.io/rtl_buddy/v5/concepts/hub/#driving-the-hub-from-the-cli-rb-hub-send). |
