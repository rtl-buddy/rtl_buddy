...
**`rb wave` bridge reports surfer disconnected** — the bridge owns the WCP TCP connection, not the hub. Check the surfer fork is on PATH and built with WCP support; see [Waveform Viewer](https://rtl-buddy.github.io/rtl_buddy/v5/concepts/wave/). The hub stays up regardless; reconnect is automatic.
