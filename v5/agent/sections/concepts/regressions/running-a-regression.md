...
Each suite's outputs land under that suite's own `tests.yaml` directory; the orchestration log and any merged coverage artifacts land under `dirname(regression.yaml)`. See [Execution Context](https://rtl-buddy.github.io/rtl_buddy/v5/concepts/execution-context/) for how the per-suite re-anchoring works.
