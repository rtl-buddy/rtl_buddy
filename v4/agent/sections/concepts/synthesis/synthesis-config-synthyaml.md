...
A synthesis can select a named **effort level** that controls how much work the flow does. Efforts are defined once in `root_config.yaml` under `cfg-synth-efforts` (see [below](https://rtl-buddy.github.io/rtl_buddy/v4/concepts/synthesis/#synthesis-effort-configuration)) and referenced per synthesis:
