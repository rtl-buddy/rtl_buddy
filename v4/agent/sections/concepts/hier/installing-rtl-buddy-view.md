...
For `--format dot` rendering to SVG/PNG, install Graphviz (`brew install graphviz` / `apt install graphviz`) and pipe the output through `dot` (see [Output formats](https://rtl-buddy.github.io/rtl_buddy/v4/concepts/hier/#output-formats) below).
