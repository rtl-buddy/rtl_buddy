...
class VlogFilelist:
    ...
    def write_output(
        ...
        path_prefix = os.path.relpath(
            os.path.dirname(self.model_cfg.get_model_path()) or ".",
            os.path.dirname(output_filepath) or ".",
        )  # Path from output dir to model dir
        model_filelist = self.model_cfg.get_filelist()

        entries = self._extract(
            model_filelist, unroll, os.path.join(path_prefix, "models.yaml")
        )
        ...
        lines = self._process(
            entries, flatten=flatten, strip=strip, deduplicate=deduplicate
        )
