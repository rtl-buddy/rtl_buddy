...
class UvmVlogPost(VlogPost):
  ...
  def get_results(self):
    ...
    results = {}
    with open(self.path, "r") as f:
      summary = re.search(r"-+\s*UVM Report Summary\s*-+\s*\**\s*Report counts by severity\s*((?:UVM_(?:INFO|WARNING|ERROR|FATAL)\s*:?\s*[0-9]+\s?)+)", f.read())

      if summary is None:
        results = { "result": "FAIL", "desc": f"No UVM Report Summary detected. See {self.path}." }
      else:
        totals = dict(map(lambda match: (match.group(1), int(match.group(2))), re.finditer(r"^UVM_(INFO|WARNING|ERROR|FATAL)\s*:?\s*([0-9]+)", summary.group(1), re.MULTILINE)))
        if "WARNING" not in totals or "ERROR" not in totals or "FATAL" not in totals:
          results = { "result": "FAIL", "desc": f"Invalid UVM Report Summary detected. See {self.path}" }
        else:
          message_summary = ", ".join(map(lambda kv: f"{kv[1]} uvm {kv[0].lower()}{'s' if kv[1] != 1 else ''}", filter(lambda kv: kv[0] != "INFO", totals.items())))
          results_str = f"{message_summary} detected. max_warnings={self.max_warns}, max_err={self.max_errors}"
          if totals["WARNING"] <= self.max_warns and totals["ERROR"] <= self.max_errors and totals["FATAL"] <= 0:
            results = { "result": "PASS", "desc": results_str }
          else:
            results = { "result": "FAIL", "desc": f"{results_str}. See {self.path}" }
