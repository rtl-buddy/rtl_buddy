...
`model_path` and XDC paths are relative to `fpga.yaml`. The model name is the synthesis top. `part` and `platform` are mutually exclusive; naming both is an exit-2 configuration error. Expected-failure fields follow [Expected Failures](expected-failures.md).
