...
`phys-model.json` and `phys-manifest.json` are the exception to the table above: with `phys-run` set they are written into the synthesis run's artefact directory instead, and everything else stays here. See [Pair the model with a synthesis run](https://rtl-buddy.github.io/rtl_buddy/dev/concepts/power/#pair-the-model-with-a-synthesis-run).
