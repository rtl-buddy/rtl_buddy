...
`rb mcp` exposes the same query builders as `cov_summary` and `cov_module`. They read files directly and do not require a running hub. See [The MCP server](https://rtl-buddy.github.io/rtl_buddy/dev/concepts/graph/#the-mcp-server).
