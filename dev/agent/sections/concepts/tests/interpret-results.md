...
A requested coverage merge that produced nothing also exits 1, even when every test passed, because the run reported an incomplete measurement. Artefacts and results are written first; see [Read a failed merge](https://rtl-buddy.github.io/rtl_buddy/dev/concepts/coverage/#read-a-failed-merge).
