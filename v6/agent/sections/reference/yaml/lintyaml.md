...
Lint uses the platform-routed `cfg-verible` entry. Model expansion drops `-v`, `-y`, and `+` directives, then applies root and check exclusions. Outputs are `artefacts/<name>/lint.f` and `lint.log`. See the [CLI reference](https://rtl-buddy.github.io/rtl_buddy/v6/reference/cli/) for commands and options.
