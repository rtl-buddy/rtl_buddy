...
* the tool set is SDK-free — it builds, lists and answers on a machine
  ...
  checkable here;
* the stateless tools are always served, and the hub tools appear only
  when a live hub was discovered, so an agent on a CI node is never
  offered a tool that can only fail;
