...
Enablement follows the peer list in the status strip, but only for `send`: it is dark when nobody is running X, because nothing would see the envelope (the tooltip points at `open ↗`). `open ↗` is live whenever the selection gives it something to send. It may take X's hub slot off a tab that already has it — [one client per origin](#peers-who-connects-to-the-hub) — but that is now a clean handover: the evicted tab stops reconnecting and offers to take the slot back, so the tooltip names the cost ("replaces the currently open X tab's hub connection") instead of the button going dark.
