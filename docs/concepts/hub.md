...
What it shows: a dashboard (per-metric scalars from the shared [coverage ramp](#design-tokens-hubthemecss), the run's provenance, observed SVA cover points), a file list ordered coldest-first for the selected metric with module and path filtering, and per-file **source annotation** — the gutter carries line hits, and the branch, toggle, expression and cover points on a line are summarised beside it. Selecting a test turns it into a lens: every number becomes that test's contribution, which is how you answer "what would I lose by dropping it".

The summary is one small badge per metric on the line — `T 12/16`, `B 3/4` — tinted on the same ramp, because one 32-bit bus declaration is 64 toggle points and a chip each pushed the code off the right of the screen. The marks column is capped, so the code stays where you can read it. Clicking a badge opens a **detail row** under that line (one at a time), and toggle points there are drawn as a **bit grid**: one square per bit of each signal, MSB left, rows of 32 with a gap at every byte, the top half of a square being `0→1` and the bottom half `1→0`. A bit covered both ways is a solid block, a half-toggled bit is visibly half, and hovering names the bit and both hit counts under the active lens. Clicking a square opens that bit's per-test attribution — the uncovered direction when only one of the two is uncovered, since that is the one you came to look at. A `cov_focus` whose `item` names a toggle point (`paddr[3]:0->1`) opens the row and highlights its square.
