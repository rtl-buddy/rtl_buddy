...
`--run-tag <tag>` moves every path in the table above under
`artefacts/.runs/<tag>/`, so two regressions can dispatch from one checkout at
once. The tag is passed to each dispatched job on its command line, so build
and simulation jobs write back into the tree their head owns. See
[Execution Context](execution-context.md#run-two-regressions-in-one-checkout).
