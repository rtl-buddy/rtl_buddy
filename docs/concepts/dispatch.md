...
- CPU efficiency is measured against the cpus the job **requested** (`ReqCPUS`), not the cpus the scheduler handed out (`AllocCPUS`).
...
Efficiency is therefore taken against `ReqCPUS`, which is what a `resources.cpus` or `compile.cpus` edit actually moves, and a `reduce` is emitted only when the suggestion is strictly below the request. `Reserved` in the table and `reserved` in `payload.reservation_advice` are that requested figure. Where the scheduler gave out more, the allocated number rides along: the table shows `4 (8 allocated)` and the finding carries an additional `allocated` field, so `squeue` and `sacct` still reconcile. Older telemetry, and any backend that reports an allocation but no request, falls back to the allocated figure and the `allocated` field is null.
