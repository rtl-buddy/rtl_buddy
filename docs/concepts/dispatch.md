...
A compile that reuses an existing build says so. `compile.build_reused` prints the reused directory and the age of its stamp at normal verbosity, and the test's `compile.log` carries the same breadcrumb, naming the command a rebuild would run — a run that compiled nothing is now visible as such instead of only as a missing file. What a stamp validates against is content: every tracked input under the project root is compared by hash, so a `preproc` hook that regenerates a file byte-for-byte reuses the build, while any real edit invalidates it even on a node whose cached `stat` still describes the file as it was.
