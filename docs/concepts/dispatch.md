...
Every run records which build it simulated, and the head cross-checks them at collect. A run's result envelope carries a `build_stamp` block naming the compile key its stamp was written for and the executable that stamp vouched for, and the build job's envelope records the same key digest beside each config it built. Runs that agree on the key but not on the executable get `dispatch.binary_mismatch` on the console: one of them rebuilt the shared directory instead of reusing it, so its neighbours may have simulated a binary that was replaced under them. It is a warning rather than a verdict, since the runs are already scored against whatever they ran.
