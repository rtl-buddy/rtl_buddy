...
At most one build job of a given identity runs at a time, guaranteed by Slurm. Each build job is named after what it builds — `rb-build-<hash>` over the suite directory and the builder selection — and submitted with `--dependency=singleton`, which defers a job until every previously launched job of the same name and owner has terminated. This is the interrupted-run case: a client that was Ctrl-C'd leaves its build job on the cluster, and the re-run's build job would otherwise Verilate into the same `artefacts/.shared-builds/obj_dir_<key>` beside it. Instead it waits for the orphan to finish — holding no allocation while it waits — then revalidates the stamp under the build lock and reuses what the orphan built. The scheduler evaluates the clause itself, so there is no window between deciding and submitting, and no dependence on the submit host being able to query the queue. Alongside it the head asks `squeue` for its own still-queued jobs of that name purely to name them: when it finds any, `dispatch.build_job_deduped` reports the wait and the ids on the console. That probe is informational — if it is unavailable or comes back empty, the line is absent and the guarantee is unchanged (`dispatch.build_dedup_unavailable` records the reason at DEBUG).

The identity is coarser than a compile key, deliberately. Compile keys fingerprint sources, flags and defines, and are only knowable after a config's `preproc` has run, inside the job — so the head names jobs by what it *can* see, and it leaves the planned tests out: `rb regression` over a suite and `rb test alpha` inside it compile the same key into the same directory, and separating them would leave exactly the "interrupt a regression, re-run one test" case racing. Two runs of one suite therefore share a name whether or not their sources have diverged, and a false match costs queue latency — the second job waits, finds the stamp stale, and rebuilds — never a wrong build. A different suite or builder never adopts the wait. A `--dependency` of your own in `sbatch-args` is composed with rather than replaced (`afterok:7,singleton`), except when it uses the any-of separator `?`: Slurm permits one separator per expression, so composing there would make sbatch reject the submission, and the dedup stands down instead (`dispatch.build_dedup_unavailable`, DEBUG) leaving the build lock to do the work. If a build job stays `PENDING` after `dispatch.build_job_deduped`, the job it is waiting for is stuck: `scancel` the ids that warning names, or `squeue --name=<job name>` to find them. The `local-parallel` backend needs none of this: its jobs are processes on one host, where the build lock already serialises them.
...
Delay for retry number `n` is `min(backoff-max-sec, backoff-sec * 2^(n-1))`, multiplied by jitter. Slurm holds retries with `--begin`; local-parallel holds them outside the worker pool. User `sbatch-args` occur last, so a user `--begin` overrides retry backoff. The one exception is the build job's `--dependency`, which is emitted after `sbatch-args` and carries the configured expression, so the shared-build dedup cannot be dropped by a dependency of your own.
