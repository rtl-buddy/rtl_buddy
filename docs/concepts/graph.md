...
An entry also carries an optional `compile` block with `duration_sec`, `builder`, and `reused` when the run's result envelope records one. Whichever path ran the compile writes it: a local run records its own, and a dispatched run's shared build job reports its record back, folded into the result at collect. Its values are read from the envelope, never measured at overlay time, and the block is absent when the envelope says nothing about the compile, so byte-stability holds for a refresh with nothing rerun.
