...
| binding | `rtl_buddy`, plus an extractor when installed ([rtl-buddy-graph-extract](https://github.com/rtl-buddy/rtl-buddy-graph-extract) or Graphify) | cocotb test to Python module to DUT, `dut.<signal>` to port, test to golden model |
...
└── graphify.log                 # the extractor's stdout/stderr, when it ran
```
...
| `--no-graphify` | Skip the extractor's binding tier without probing for one |
| `--graphify-llm` | Opt into Graphify's LLM pass (off by default; the bundled extractor rejects it) |
| `--no-graphify-cross-check` | Don't run the extractor's `merge-graphs` as a second opinion |
...
Two tools satisfy the extract contract, resolved in this order: [Graphify](https://github.com/safishamsi/graphify) — the contract's origin, which wins when someone has gone out of their way to install it — and [rtl-buddy-graph-extract](https://github.com/rtl-buddy/rtl-buddy-graph-extract), the bundled clean-room implementation (stdlib-only, deterministic, installed with `uv sync --extra graph-extract`, resolved from git until its first PyPI release). The fingerprint records which one ran, so swapping implementations invalidates the cached build.

Graphify's semantic pass ships file content to whatever model it is configured with, so it is never run implicitly: only `--graphify-llm` turns it on. The bundled extractor has no semantic pass and *rejects* the flag rather than silently ignoring it — a failed tier is more honest than a pass that never happened.
...
When an extractor *is* installed, its `merge-graphs` verb runs over the same per-tier files as a cross-check and the comparison lands in `graph-meta.json` under `merge.graphify_cross_check`. A disagreement is reported, never adopted.
