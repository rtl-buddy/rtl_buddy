...
- **`cfg-surfer`** — the entry the active platform routes via `cfg-platforms[].surfer` (falling back to `surfer-default` when nothing is routed) has its resolved path added similarly.
- **`cfg-{synth,pnr,power,cdc,fpv,fpga}-tools`** — when `cfg-platforms` routes one of these blocks, the routed entry's executable is added to the matching tool's detector chain, matched by the binary's basename (`/opt/rb-tools/bin/yosys` → the `yosys` spec). Unrouted blocks are left alone: their entry is chosen per run by the flow's own YAML, which is not a per-platform statement about where the binary lives.
