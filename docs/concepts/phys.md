...
`rb power` records its **mode** — `static` or `dynamic` — and a compact **activity** block: the resolved source (`default`, `synthetic`, `saif` or `vcd`), the trace, the test whose artefact directory the trace came out of, the scope, and the toggle and duty a synthetic run used. Each detail belongs to the source that consumed it: a `default` or `synthetic` run records no trace, test or scope even when its config still carries `activity.saif` and `activity.scope`, because the analysis read neither. Without them a µW figure carries no statement of what it measures: leakage plus internal and a SAIF-driven total print in the same column. A `saif` run records no toggle and duty, because a trace is what drove it. The test name is derived from the layout — `verif/demo/artefacts/csr_smoke/dump.saif` was produced by `csr_smoke` — and is absent for a trace kept anywhere else.

Both flows record a **config fingerprint**: the platform, the effort, the constraints file and its sha256, and a short digest of the effective tool options. The digest covers what the backend resolved — tool defaults with the effort's `synth-args`/`abc-args` and the per-synthesis `tool_overrides` already folded in, plus the elaboration parameters and defines for a synthesis, and the tool, netlist source, mode, activity source and register level for a power run — what the backend resolved and used, so a field no backend reads (a power run's `tool_overrides`) is left out rather than reporting a difference the numbers cannot have. The constraints are the SDC the run actually read: a `netlist-source: pnr` run with no explicit `constraints:` records the router's `<top>.routed.sdc` it was given. It is taken over the *effective values*, rendered as canonical JSON, not over the files they came from, so two configs that spell one setting differently and resolve to the same options are one experiment. As a summary it reads `nangate45 · timing-opt · sdc a1b2c3d4 · opts 5655beea1f20`.

A manifest under `artefacts/xplr/<exp-id>/` carries that **experiment id** in every payload's run header and in the run listing. The id comes from the path, so it is there before the experiment's `record.json` exists; the record is read only for the experiment's `hypothesis`, which becomes the entry's label. A malformed record costs the label and nothing else. The ledger's own `worktrees/` directory is not an experiment.
