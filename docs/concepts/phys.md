...
`rb mcp` exposes the same query builders as `phys_summary`, `phys_module`, and `phys_instance`. They read files directly, run no EDA tool, and do not require a running hub. `phys_dir` and `manifest` are the tool arguments for `--phys-dir` and `--manifest`; a relative path resolves against the project root. `phys_summary` takes the same `limit` the flag does; `phys_module` and `phys_instance` take none and so always return the complete list. A `phys_focus` tool mirroring `rb hub send phys-focus` joins them when a live hub is discovered. See [The MCP server](graph.md#the-mcp-server).
