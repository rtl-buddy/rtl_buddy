...
Both flows record a **config fingerprint**: the platform, the effort, the constraints file and its sha256, and a short digest of the effective tool options. The digest covers what the backend resolved — tool defaults with the effort's `synth-args`/`abc-args` and the per-synthesis `tool_overrides` already folded in, plus the elaboration parameters and defines for a synthesis, and the tool, netlist source, mode, activity source, register level and tool overrides for a power run. It is taken over the *effective values*, rendered as canonical JSON, not over the files they came from, so two configs that spell one setting differently and resolve to the same options are one experiment. As a summary it reads `nangate45 · timing-opt · sdc a1b2c3d4 · opts 5655beea1f20`.
