...
- **Cover points** (`test`, `regression`): where `functional` is one scalar for the whole run, `covers` names the individual points behind it — a list of `{name, file, line, hits}`, present both on each `payload.results` entry (that test's counts) and on `payload.coverage` (summed across the run). `name` is the SVA `cover property` label, which is what lets you grade *which* verification-plan items a suite exercised rather than just how many. `hits` sums every instance of a source point, so treat `hits > 0` as "covered" — Verilator already folds a point's instances together before writing the database, so a point instantiated three times arrives as one entry with the counts added up. The list normally lines up with `functional`: same points, same hit/total. The one case where it doesn't is a cover property compiled into more than one module (typically one living in an `include`d file) — Verilator keys those separately per module, while `covers` folds them into a single entry by `(file, line, name)`, so the list is shorter than the `functional` denominator there. Unlike the metrics above this needs no `--coverage-merge*` flag — the points are read from the per-test raw databases — and `file` is the path as the simulator recorded it, which may be relative to the run directory. Verilator only: on other simulator families the field is absent, which means "not collected", not "nothing covered".
