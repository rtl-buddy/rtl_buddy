...
cfg-dispatch:                 # optional; remote execution for regression/randtest
  backend: slurm             # default: local (in-process)
  resources: { cpus: 2, mem: 4G, time: "01:00:00" }   # sim jobs
  compile: { cpus: 8, mem: 16G, time: "02:00:00" }   # build job (defaults to resources)
...
- `cfg-dispatch` (optional) configures remote test execution for `rb regression --dispatch slurm` and `rb randtest --dispatch slurm`. `backend` is `local` (default, in-process) or `slurm`. `resources` are the per-sim-job reservation defaults (`cpus`/`mem`/`time`), and `compile` is the reservation for the dispatched build job (defaults to `resources` — a large Verilation is often heavier than the sims); **quote `time`** — an unquoted `4:00:00` is YAML-1.1 sexagesimal (the integer 14400). `sbatch-args` pass to `sbatch` verbatim (partition/account/qos). `max-jobs-per-array` caps concurrent elements per submitted array (peak concurrency ≈ that × number of arrays); `poll-interval` (>0) is the queue-poll cadence. `rightsize` tunes reservation right-sizing advice (`report` on by default; `over-threshold`/`near-limit` utilization bounds; `margin` for the suggested value). Per-test overrides use the same `resources:` shape in `tests.yaml`. Full guide: [Parallel Dispatch (Slurm)](../concepts/dispatch.md).
...
| `resources` | dict | Optional per-test job reservation for `--dispatch slurm` (`cpus`/`mem`/`time`). Layered over the testbench's `resources:` and `cfg-dispatch.resources`, field by field. **Quote `time`** (`time: "04:00:00"`). Also settable on a `testbenches[]` entry. See [Parallel Dispatch (Slurm)](../concepts/dispatch.md). |
