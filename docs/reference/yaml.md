...
| `sbatch-args` | Empty list; appended verbatim and therefore overrides duplicate generated flags. A `--cpus-per-task` here supersedes the resolved `cpus`, so CPU right-sizing falls back to the scheduler's `ReqCPUS` for that run |
