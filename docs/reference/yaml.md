...
| `top` | Optional | Root module of the filelist when it is not named after the model; default `name` |

`top` is the model's root module everywhere rtl_buddy elaborates it: `rb hier`, `rb hier-query`, `rb axi-profile`, `rb graph build`, and the default top of a `cdc.yaml`, `synth.yaml`, `lint.yaml`, `fpga.yaml`, `fpv.yaml`, or `mut.yaml` run against the model. A run's own `top:` still wins.

Set `graph: false` for a model with no elaborable root — an SV `interface` published as a library entry, or a filelist of vendored IP with no module named after the model. `rb graph build` then records the model, and any testbench or non-simulation run rooted at it, under the design tier's `skipped` list instead of attempting an export that can only fail. The config tier still emits the model node, so `spec:` and test cross-references keep resolving; it carries `graph: false` and no `maps_to` edge. Prefer `top:` when the filelist does elaborate and only the root module name differs.
