...
| `top` | Optional | Root module of the filelist when it is not named after the model; default `name`. Must be a simple SystemVerilog identifier and unique across the graphable models `rb graph build` selects |
...
`top` is checked at load time too, against a stricter rule: it must be a simple SystemVerilog identifier (a letter or underscore, then letters, digits, underscore or `$`). It does not stay in HDL — the FPGA flows name the bitstream `<top>.bit`, and the Yosys, Vivado and OpenROAD generators interpolate it into Tcl — so a value carrying a path separator, a newline or a shell or Tcl metacharacter is refused rather than escaped per tool. SystemVerilog escaped identifiers (`\name `) are legal HDL and refused here as well: no flow can name a file or a Tcl token after one safely.
