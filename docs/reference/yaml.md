...
| `sbatch-args` | Empty list; appended verbatim and therefore overrides duplicate generated flags. Any argument here that sets the job's cpu request — `-c`/`--cpus-per-task`, or the task/node counts `ReqCPUS` multiplies it by (`-n`/`--ntasks`, `--ntasks-per-*`, `-N`/`--nodes`) — supersedes the resolved `cpus`, so CPU right-sizing falls back to the scheduler's `ReqCPUS` for that run and its `cpus` advice names this key rather than the masked `resources.cpus` / `compile.cpus`. Within one option the last occurrence wins, as it does for sbatch; distinct options (`--ntasks` and `--cpus-per-task`) multiply instead, and the advice then names the product rather than any one of them. Only a lone `-c`/`--cpus-per-task` is offered the suggested value; the task/node counts are told to be decomposed. An override also disables the compile `cpus` floor, which bounds a reservation sbatch never saw. Node-selection constraints (`--threads-per-core`, `-B`) and `--exclusive` are not overrides — the generated `--cpus-per-task` still states the request; `--cpus-per-gpu` is not either, since Slurm rejects it alongside the `--cpus-per-task` every job carries |
