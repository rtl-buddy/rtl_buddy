...
| `rb hier`, `rb hier-query` | Pluggable — curated | [rtl-buddy-view](https://github.com/rtl-buddy/rtl-buddy-view) | `uv tool install rtl-buddy-sch` (≥ 0.3.0 for the `hier-query` verbs; the distribution was renamed from `rtl-buddy-view`, which is frozen at 0.5.0 — the executable is still `rtl-buddy-view`). Optional: `graphviz` (`dot`) for `--format dot` → SVG/PNG; `pyslang` for `--frontend slang`. See [Hierarchy Rendering](concepts/hier.md). |
| `rb graph build` | Pluggable — curated | [rtl-buddy-view](https://github.com/rtl-buddy/rtl-buddy-view), optional [rtl-buddy-graph-extract](https://github.com/rtl-buddy/rtl-buddy-graph-extract) | The design tier needs `rtl-buddy-view` with the `graph` verb (≥ 0.4.0); an older viewer fails that tier with an upgrade hint. The binding-tier extractor is **optional** — without it the merged graph still carries the design + config tiers and the binding tier is reported as skipped. Install it with `uv pip install git+https://github.com/rtl-buddy/rtl-buddy-graph-extract` (no PyPI release yet — the first one is tracked in [rtl-buddy-graph-extract#1](https://github.com/rtl-buddy/rtl-buddy-graph-extract/issues/1), after which this becomes a normal `rtl_buddy[graph-extract]` extra). Gate with `rb tool-check --explain rtl-buddy-view` / `--explain rtl-buddy-graph-extract`. See [Design Knowledge Graph](concepts/graph.md). |
