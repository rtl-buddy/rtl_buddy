...
| `rb graph build` | Pluggable — curated | [rtl-buddy-view](https://github.com/rtl-buddy/rtl-buddy-view), optional `graphify` | The design tier needs `rtl-buddy-view` with the `graph` verb (≥ 0.4.0); an older viewer fails that tier with an upgrade hint. Graphify is **optional** — without it the merged graph still carries the design + config tiers and the binding tier is reported as skipped. Gate both with `rb tool-check --explain rtl-buddy-view` / `--explain graphify`. See [Design Knowledge Graph](concepts/graph.md). |
