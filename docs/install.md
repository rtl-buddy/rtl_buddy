...
| `rb fpga`, `rb fpga-regression` | Pluggable | AMD/Xilinx Vivado | Proprietary; driven in non-project batch mode (`vivado -mode batch`). Optional feature — runs report SKIP when `vivado` is absent; larger parts need a purchased license. See [FPGA Implementation](concepts/fpga.md). |
