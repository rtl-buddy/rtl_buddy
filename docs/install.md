...
| `rb regression --dispatch slurm` | Optional external | Slurm client (`sbatch`/`squeue`/`scancel`) | Only for dispatching regression jobs to a Slurm cluster; the default `--dispatch local` needs nothing extra. Requires a shared filesystem between submit host and compute nodes. No native macOS build — use a Linux submit host or a single-node Slurm-in-Docker for local testing. See [Regressions → Parallel dispatch](concepts/regressions.md#parallel-dispatch-via-slurm). |
