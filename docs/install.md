...
| `rb graph build` | Pluggable — curated | [rtl-buddy-view](https://github.com/rtl-buddy/rtl-buddy-view), optional binding-tier extractor | The design tier needs `rtl-buddy-view` with the `graph` verb (≥ 0.4.0); an older viewer fails that tier with an upgrade hint. The binding-tier extractor is **optional** — without one the merged graph still carries the design + config tiers and the binding tier is reported as skipped. The bundled [rtl-buddy-graph-extract](https://github.com/rtl-buddy/rtl-buddy-graph-extract) installs with the `[graph-extract]` extra (`uv sync --extra graph-extract`; git-resolved until its first PyPI release); the original `graphify` is also recognized and wins when both are present. Gate with `rb tool-check --explain rtl-buddy-view` / `--explain rtl-buddy-graph-extract`. See [Design Knowledge Graph](concepts/graph.md). |
