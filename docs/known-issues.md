...
Two kinds of name are left out of the walk. **Directories**: dot-directories (`.git`, `.svn`) and RTL Buddy's own managed trees — the suite's `artefacts/`, `.shared-builds/`, and any `obj_dir*`. The artefact prune matters for correctness, not just cost: an `+incdir+` that is an ancestor of the suite's `artefacts/` — `+incdir+.` declared in a `tests.yaml`, or `+incdir+..` from a design directory that holds verif suites — would otherwise stamp `run.f`, the compile log, the build output and the stamp itself, all of which are written *after* the fingerprint that lists them, so no later process could ever validate the stamp and every dispatched simulation job would recompile. A project directory genuinely named `artefacts` or `obj_dir*` is not tracked. **Files**: a denylist of editor and VCS bookkeeping (`.DS_Store`, `.gitignore`, `.gitattributes`, `.gitkeep`, `*.swp`, `*.swo`, `*~`, `.#*`, `#*#`). Every other file is listed, dot-prefixed ones included, because `` `include ".config.svh" `` is legal and resolves.
