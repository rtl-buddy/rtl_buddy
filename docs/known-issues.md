...
Outputs named after the design's top — `<top>.bit`, `<design>.routed.odb` — are matched by suffix rather than by name, so editing a run's `model:` or `top:` does not strand the previous top's files in the same directory. rtl_buddy's own bookkeeping is never matched this way: a run's `result.json` and the dispatch envelopes survive, which matters because an artifact directory is keyed on a run's *name* and names need not be unique across commands — an `rb fpga` run and a simulation test called the same thing share one directory. An artifact directory belongs to one run, so everything carrying a suffix that flow writes is that flow's own output; nothing else in the directory is touched, and the scan never recurses into a workdir a tool owns.
