...
One suite build job compiles every unique compile key, and all simulation jobs wait for the complete build. `compile.parallel` — the suite's own `compile:` block where it sets one, otherwise `cfg-dispatch.compile.parallel` — compiles that many distinct builds at once inside that job, so `compile.time` must cover the longest batch rather than the serial total, and `compile.mem` must cover that many concurrent elaborations because only `cpus` is scaled for you. Count `compile.start` events to estimate the work.
