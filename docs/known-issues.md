...
Under `--dispatch slurm` a second run of the same suite does not create that contention in the first place: the head names each build job after what it builds and depends on any job of that name already in its own queue, so an interrupted run's orphaned build job is waited on rather than raced (`dispatch.build_job_deduped`). Three bounds. The name is derived from the suite, the planned test names and the builder selection, not from the compile keys — those are only knowable inside the job — so an edit between two runs makes the second wait for a build it then rebuilds, costing queue latency. The probe covers only your own jobs, so two *users* building into one shared tree still meet at the lock. And a submit host where `squeue` is absent or errors submits as before, recording `dispatch.build_dedup_unavailable` at DEBUG. Unshared builds — a builder that cannot share, or `--share-build` declined — take no build lock at all: within one run each per-test build directory has a single writer, but two concurrent runs of such a suite still write one directory, and only the Slurm dedup above stands between them. Interrupting a run still leaves its jobs on the cluster and a re-run does not attach to them: the dedup makes the re-run's build job wait for the orphaned build, but the orphan's simulation jobs run to completion on their own.
