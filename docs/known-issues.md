...
## An unknown `synth.yaml` `tool_overrides` key warns but does not stop the run

`synth.yaml` `tool_overrides.<tool>` keys are the **snake_case** attribute names (`synth_args`, `abc_args`, `strategy`, `frontend`, `plugin_path`, `single_unit`), not the kebab-case spelling used under `cfg-synth-tools.opts`. A key outside that set — a typo, or `single-unit` / `plugin-path` copied from root-config — is reported at config resolution as a `synth_tool_config.unknown_override` **WARNING** (with a `'single-unit' -> 'single_unit'` hint) and then **ignored**: the run proceeds with the tool-level default for that field, so a synthesis that looks fine in the summary may not have used the option you set. Through v6.36.0 the key was dropped with no message at all. It warns rather than fails because breaking changes only land on major bumps (see [Migrations](migrations.md)); making it fatal is a candidate for the next major. Two related shapes *are* fatal today, since no existing config could have carried them: a wrongly-typed `single_unit` (`"true"`, `1`, or an empty value — write an unquoted YAML `true`/`false`) and a `tool_overrides.<tool>` block that is not a mapping. See [Per-tool overrides](concepts/synthesis.md#per-tool-overrides).
