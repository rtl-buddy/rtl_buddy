...
Outputs named after the design's top — `<top>.bit`, `<design>.routed.odb` — are matched by suffix rather than by name, so editing a run's `model:` or `top:` does not strand the previous top's files in the same directory. Another command's durable outputs are never matched this way either: a run's `result.json`, the dispatch envelopes, and the sibling commands' reports — `cdc.json`, the CDC domain maps, `power.rpt`, the synthesis netlists — all survive. This matters because an artifact directory is keyed on a run's *name* and names need not be unique across commands: an `rb fpga` run, a CDC analysis and a simulation test called the same thing all share one directory.
