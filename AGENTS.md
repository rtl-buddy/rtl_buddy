...
- Main-branch documentation deployments update only `dev`. Stable release deployments rebuild every published major from its latest stable tag after the new tag exists and refresh `dev` from the latest `origin/main` snapshot so its version selector includes a newly added major without regressing development content.
