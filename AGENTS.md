...
- `rtl_buddy.graph.config_tier` extracts the **config tier** of the design knowledge graph (#375/#376) by reading the existing loaders and `tools/spec_trace.py` — never a second YAML parser, so the graph cannot disagree with `rb spec check-coverage` / `check-design`. It emits NetworkX node-link JSON; the node/edge vocabulary, the id conventions, and the `module:<name>` stitch to rtl-buddy-view's design tier are documented in `docs/concepts/graph.md`. The entry points are `build_config_tier()` / `extract_config_tier()`.
